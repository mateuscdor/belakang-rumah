...
let session = `./src/session/${setting.sessionName}.json`
const { state, saveState } = useSingleFileAuthState(session)

function title() {
      console.clear()
	  console.log(chalk.bold.green(figlet.textSync('ERLANG', {
		font: 'Standard',
...
const connectToWhatsApp = async () => {
	...
	conn.ev.on('connection.update', (update) => {
		...
		if (connection === 'close') {
			...
			reconnect.stop()
			starting.stop()
			console.log(mylog('Server On'))
			lastDisconnect.error?.output?.statusCode !== DisconnectReason.loggedOut 
			? connectToWhatsApp()
			: console.log(mylog('Koneksi Server Putus!'))
		}
	})
