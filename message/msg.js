...
const { waterMark } = require("../src/waterMark.json");
const { buttonGroup } = require("../src/buttonGroup.json");
...
module.exports = async(conn, msg, m, setting) => {
	try {
		...
		const buttonsDefault = [
			{ callButton: { displayText: `Contact Developer`, phoneNumber: `17375005660` } },
			{ urlButton: { displayText: `Group`, url : `${buttonGroup}` } },
		...
		switch(command) {
			...
			case prefix+'help':
			    ...
			    conn.sendMessage(from, { caption: teks, location: { jpegThumbnail: fs.readFileSync(setting.pathimg) }, templateButtons: buttonsDefault, footer: `${waterMark}`, mentions: [sender] })
				break
			case prefix+'runtime':
			...
			case prefix+'ytmp3': case prefix+'mp3':
			    ...
			    break
			case prefix+'getvideo': case prefix+'getv':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
		        ...
		        break
			case prefix+'getm': case prefix+'getmusic':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
			    if (!isQuotedImage) return reply(`Balas hasil pencarian dari ${prefix}ytsearch dengan teks ${command} <no urutan>`)
				if (!quotedMsg.fromMe) return reply(`Hanya bisa mengambil hasil dari pesan bot`)
				if (args.length < 2) return reply(`Balas hasil pencarian dari ${prefix}ytsearch dengan teks ${command} <no urutan>`)
			    ...
			    break
			case prefix+'ig': case prefix+'instagram': case prefix+'ig':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
				if (args.length < 2) return reply(`Kirim perintah ${command} link`)
			    if (!isUrl(args[1])) return reply(mess.error.Iv)
			    ...
			    break
			case prefix+'facebook': case prefix+'fb':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
			    if (args.length < 2) return reply(`Kirim perintah ${command} link`)
			    if (!isUrl(args[1])) return reply(mess.error.Iv)
 ...
 *${prefix}getvideo <no urutan>*
 Untuk mengambil Audio/Video dari hasil pencarian`
            ...
            case prefix+'tf':{
                 if (args.length < 2) return reply(`Kirim perintah *${command}* @tag nominal\nContoh : ${command} @17375005660 2000`)
